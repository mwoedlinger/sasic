...
def test(device, resume):
    ...
    with torch.no_grad():
        for sample, idx in tqdm(test_loader):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Order of arguments: gpu_idx restore_path. Restore_path should point to a directory (!) containing the model weights "model.pt" (i.e. the path that is printed after evaluation epochs during training)')
    parser.add_argument('argv', nargs='*', help='gpu_idx resume')
    parser.add_argument('--test', default='cityscapes', 
                        help=f'name of test dataset. Options (includes stereo image datasets): {", ".join(list(data_zoo_stereo.keys()))}')
